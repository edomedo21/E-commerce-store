...
export const SettingsForm: React.FC<SettingsFormProps> = ({
    ...
    const onDelete = async () =>{
        try{
            ...
            router.refresh();
            router.push('/');
            toast.success("Trgovina obrisana!");
        }
        catch(error){
            toast.error("Da li ste uklonili sve proizvode i kategorije?");
        }
        finally{
            setLoading(false);
            setOpen(false);
...
return (
    ...
    <div className="flex items-center justify-between">
        <Heading title="Postavke" description="Upravljajte trgovinom"/>
            ...
            <div className="grid grid-cols-3 gap-8">
                ...
                render={({field})=>(
                    <FormItem>
                        ...
                        <FormControl>
                            <Input disabled={loading} placeholder="Ime trgovine..." {...field}/>
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                )}
